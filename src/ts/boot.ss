...
(define (ono-menu . content)
  (set! *ono-menu* content)
...
(define (offlineimap account args)
  ...
  (let* ([args (if (string? args) args "")]
		 ;; if account part was empty, do not specify '-a' option so
		 ;; offlineimap pulls for all accounts
		 [account (if account
					(string-append " -a " account)
					" ")]
		 [args (if args args "")]
		 ;; offlineimap reports to stderr, so we have to redirect it to stdout
		 [cmd  (string-append *offlineimap-path*
							  " "
							  *offlineimap-args*
							  " "
							  args account
							  " 2>&1")])
	(if (closure? *ono-on-stdout*)
	  (system-with-reader cmd *ono-on-stdout*)
	  (system cmd)))
